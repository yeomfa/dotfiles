...
function send_notification {
    ...
    dunstify -t $notify_timeout -r 2593 -u normal "$icon_audio_vol$padding$bar  $volume%"
...
function get_mic_toggle {
    ...
    if [ "$channels" -eq "$num_on" ];then
        dunstify -t $notify_timeout -r 2593 -u normal "$icon_capture_on${padding}On"
    # If the number of off match the number of channels, we know the mic is off
    elif [ "$channels" -eq "$num_off" ];then
        dunstify -t $notify_timeout -r 2593 -u normal "$icon_capture_off${padding}Off"
    # If the number of on and off don't match the number of channels, we don't really know the status
    else
        dunstify -t $notify_timeout -r 2593 -u normal "$icon_capture_unk${padding}Unknown"
    fi
}
